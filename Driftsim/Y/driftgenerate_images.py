...
#PHI = 2*Pi*(-S-(q/(2*Pi))*(-2*np.arctan(np.sqrt((1-epsilon)/(1+epsilon))*np.tan(ETA/2))))
    ...
    levels = np.linspace(0.5, 1., 180)
    myplot24 = plt.contourf(x_grid[:,50,:],z_grid[:,50,:],grid_z1[:,50,:,0],levels=levels)
    cbar = plt.colorbar(mappable=myplot24)
    name = '/home/aske/Driftsim/Y/Images/pardiffusiontest' + str(tstep) + '.png'
    plt.savefig(name)
    plt.clf()
