# -*- coding: utf-8 -*-
"""
Created on Wed Aug 10 14:26:07 2016
...
nz = 22 #Number of z points
ny = 128 #Number of y points
nx = 1 #Number of x points
MXG = 0 #Number of guardcells in the x-direction
Ly = 1.0#Length of y
Lx = 1.0#Length of x
...
Thetamesh = Thetause[0,:] #Creating a vector from the matrix
...
#PHI = 2*Pi*(-S-(q/(2*Pi))*(-2*np.arctan(np.sqrt((1-epsilon)/(1+epsilon))*np.tan(ETA/2))))
    ...
    name = '/home/aske/test/ThreeDdiffusion/Y/Images/22z3Dpardiffusiontest' + str(tstep) + '.png'

    fig = plt.figure()
    ax = fig.gca(projection='3d')
    ax.set_zlim3d(-4,4)
    ax.scatter(X_RT,Y_RT,Z_RT,c=N[time,:,:,:], vmin = 1, vmax = 1.25)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Toroidal Grid',fontsize=14, fontweight='bold')
    plt.savefig(name)
